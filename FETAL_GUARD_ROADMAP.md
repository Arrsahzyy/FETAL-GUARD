...
- Device UID matching is no longer the only device identity control: each device now carries a symmetric packet signing key issued by `POST /devices/{id}/signing-key` and disclosed once. Firmware signs `FGSIG1|uid|boot_id|sequence|captured_at_ms|schema|sha256(channels)` with HMAC-SHA256, and the ingestion endpoint refuses any packet from a provisioned device whose signature it cannot reproduce. `REQUIRE_DEVICE_PACKET_SIGNATURE` is mandatory in production, so an unprovisioned device is rejected there. This closes UID spoofing at the API boundary; it is not BLE bonding, and the firmware side stays unverified until compile/flash.
- Session clinical values are now derived by the backend from stored raw channels (`services/signal_processing.py`, `services/vitals_derivation.py`) instead of being left empty: the ingestion schema still refuses client-supplied summaries, so FHR, maternal HR, and signal quality come from the server's own reading of the samples it holds. Derivation requires telemetry v2 — a 1 Hz v1 snapshot cannot resolve a heartbeat — and records `derivation_status` so the UI can separate "still collecting" from "signal not good enough". **These estimators are unvalidated against CTG, Doppler, or any reference signal; they are technical estimates, not clinical measurements.**
- Alerts now have a real producer (`services/alerting.py`). Rules fire only from backend-derived values, only above a signal-quality floor, and at most one open alert per rule per session, using the display reference ranges in `AGENTS.md` (FHR 110–160, maternal 60–100) with screening language. Before this, the whole alert lifecycle existed but nothing ever created a `Notification`, so no clinician could receive an alert regardless of sensor readings.
